...
export async function GET(_request: Request, context: RouteContext<'/admin/api/events/[weekId]/polls'>) {
  const { weekId } = await context.params;
  return handleAdminPollsGET(weekId);
}

export async function POST(request: Request, context: RouteContext<'/admin/api/events/[weekId]/polls'>) {
  const { weekId } = await context.params;
